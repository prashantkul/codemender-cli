...
Runs a `cm find` scan and reports what it discovered. Arguments: `$ARGUMENTS`
= an optional target path (file or directory) followed by optional free-text
context to steer the scan (e.g. `routes/login.ts focus on auth bypass`).
...
2. **Parse arguments.** From `$ARGUMENTS`, take the first token that looks like a path
   as the scan target; treat the rest as context. If no path is given, use `.` (current
   dir) — but warn the user that scanning a large tree is slow and suggest a specific
   file/dir.
...
5. **Summarize.** Run `"$CM" report` and present:
   ...
   End with the next step: `/cm-fix <id>` to patch one, or `/cm-audit` to fix a batch.
