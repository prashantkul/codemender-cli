...
VCS + build wired up. Arguments: `$ARGUMENTS` = optional path to the target project
...
6. **Confirm.** Re-run `cm init --verify` and report an all-green (or note remaining
   warnings). Tell the user they can now `/cm-scan <path>`.
...
- If the user is onboarding a *vulnerable demo app* (Juice Shop, NodeGoat, DVWA…), clone it
  into the project and point `project_paths` at it, then `/cm-scan` a known-vulnerable file.
